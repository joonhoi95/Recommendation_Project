...
book_data = pd.read_csv('C:/Users/junho park/Desktop/Python/Reco_Engine/res/Preprocessed_data.csv')
        ...
        if book_title in rare_books:
            ...
            for i in range(len(books)):
                ...
                axs[i].set_title('Rating: {}'.format(round(df[df['book_title'] == books[i]]['rating'].mean(), 1)) \
                                 ...
                                 + '\n' + 'title: {}'.format(common_books.loc[common_books['book_title'] == books[i], 'Category'][:1].values[0]) ,
                                 y=-0.3,
                                 color="blue",
                                 fontsize=12)
                fig.show()
...
#sns.heatmap(cosine_sim, xticklabels=common_books['combined_features'], yticklabels=common_books['combined_features'], cmap='viridis')
#plt.show()